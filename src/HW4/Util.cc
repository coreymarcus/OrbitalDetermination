...
namespace Util {
	...
	Eigen::Vector3d ECEF2ECI(Eigen::Vector3d pos, double time){
		...
		//first, find the change in time since J2000.0
		double tsinceJ2000 = (2458088.500555556 - 2451545.0)/36525.0;
		...
		double zeta = zeta_deg*M_PI/180.0;
		...
		// P(0,1) = -sin(zeta)*cos(theta)*cos(z) - cos(zeta)*sin(z);
		// P(0,2) = -sin(theta)*cos(z);
		// P(1,0) = cos(zeta)*cos(theta)*sin(z) + sin(zeta)*cos(z);
		// P(1,1) = -sin(zeta)
